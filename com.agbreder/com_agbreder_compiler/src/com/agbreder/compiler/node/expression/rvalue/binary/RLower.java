 ...
 */
public class RLower extends RBinary {
   ...
   * @throws IOException
   */
  public RLower(AGBDataInputStream input) throws IOException {
   ...
   * @param right
   */
  public RLower(RToken token, RValue left, RValue right) {
  ...
  public void build(AGBCompilerOpcode opcodes) throws Exception {
    if (this.value != null) {
      ...
      if (this.getLeft().getType().isNumber()) {
        opcodes.opNumLowerThan();
      }
      else if (this.getLeft().getType().isString()) {
        opcodes.opStrLowerThan();
      }
    }
  }
