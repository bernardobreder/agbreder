 ...
 */
public class RGreater extends RBinary {
   ...
   * @throws IOException
   */
  public RGreater(AGBDataInputStream input) throws IOException {
   ...
   * @param right
   */
  public RGreater(RToken token, RValue left, RValue right) {
  ...
  public void build(AGBCompilerOpcode opcodes) throws Exception {
    if (this.value != null) {
      ...
      if (this.getLeft().getType().isNumber()) {
        opcodes.opNumGreaterThan();
      }
      else if (this.getLeft().getType().isString()) {
        opcodes.opStrGreaterThan();
      }
    }
  }
