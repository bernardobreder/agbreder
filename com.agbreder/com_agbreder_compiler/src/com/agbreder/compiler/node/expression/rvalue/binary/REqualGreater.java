 ...
 */
public class REqualGreater extends RBinary {
   ...
   * @throws IOException
   */
  public REqualGreater(AGBDataInputStream input) throws IOException {
   ...
   * @param right
   */
  public REqualGreater(RToken token, RValue left, RValue right) {
  ...
  public void build(AGBCompilerOpcode opcodes) throws Exception {
    if (this.value != null) {
      ...
      if (this.getLeft().getType().isNumber()) {
        opcodes.opNumEqualGreaterThan();
      }
      else if (this.getLeft().getType().isString()) {
        opcodes.opStrEqualGreaterThan();
      }
    }
  }
