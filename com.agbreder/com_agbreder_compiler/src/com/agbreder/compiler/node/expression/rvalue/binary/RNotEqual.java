 ...
 */
public class RNotEqual extends RBinary {
   ...
   * @throws IOException
   */
  public RNotEqual(AGBDataInputStream input) throws IOException {
   ...
   * @param right
   */
  public RNotEqual(RToken token, RValue left, RValue right) {
  ...
  public void build(AGBCompilerOpcode opcodes) throws Exception {
    if (this.value != null) {
      ...
      if (this.getLeft().getType().isNumber()) {
        opcodes.opNumNotEqual();
      }
      else if (this.getLeft().getType().isString()) {
        opcodes.opStrNotEqual();
      }
    }
  }
